...
function Receipts({data,mutate,setMessage}:{data:FamilyData;mutate:(f:(d:FamilyData)=>void)=>void;setMessage:(s:string)=>void}){
  ...
  const analyze=async(files:File[])=>{if(!files.length)return;setBusy(true);try{const parts:ReadReceipt[]=[];for(let index=0;index<files.length;index++){setMessage(`Lendo imagem ${index+1} de ${files.length}…`);parts.push(await readReceipt(files[index]))}const best=parts.slice().sort((a,b)=>(b.confianca||0)-(a.confianca||0))[0]||{};setDraft({...best,estabelecimento:parts.find(x=>x.estabelecimento)?.estabelecimento,data:parts.find(x=>x.data)?.data,total:parts.slice().reverse().find(x=>Number(x.total)>0)?.total,itens:parts.flatMap(x=>x.itens||[]),observacoes:parts.flatMap(x=>x.observacoes||[])});setMessage(`${files.length} imagem(ns) lida(s). Confira os dados antes de salvar.`);}catch(e){setMessage((e as Error).message)}finally{setBusy(false)}};
