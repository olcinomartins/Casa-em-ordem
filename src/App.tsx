...
const nav: [Page, string, typeof BarChart3][] = [
  ...
  ["supermercado", "Supermercado", ReceiptText],
];
...
function Receipts({data,mutate,setMessage}:{data:FamilyData;mutate:(f:(d:FamilyData)=>void)=>void;setMessage:(s:string)=>void}){
  ...
  const analyze=async(files:File[])=>{if(!files.length)return;setBusy(true);const parts:ReadReceipt[]=[];const failures:string[]=[];for(let index=0;index<files.length;index++){setMessage(`Lendo imagem ${index+1} de ${files.length}…`);try{parts.push(await readReceipt(files[index]))}catch(error){failures.push(`imagem ${index+1}: ${(error as Error).message}`)}if(index<files.length-1)await new Promise(resolve=>setTimeout(resolve,1200))}if(parts.length){const best=parts.slice().sort((a,b)=>(b.confianca||0)-(a.confianca||0))[0]||{};setDraft({...best,estabelecimento:parts.find(x=>x.estabelecimento)?.estabelecimento,data:parts.find(x=>x.data)?.data,total:parts.slice().reverse().find(x=>Number(x.total)>0)?.total,itens:parts.flatMap(x=>x.itens||[]),observacoes:[...parts.flatMap(x=>x.observacoes||[]),...failures]});setMessage(`${parts.length} de ${files.length} imagem(ns) lida(s).${failures.length?" As demais podem ser reenviadas.":" Confira os dados."}`)}else setMessage(failures[0]||"Nenhuma imagem pôde ser lida.");setBusy(false)};
  const save=()=>{if(!draft)return;const receipt:Receipt={...audit(),store:draft.estabelecimento||"Estabelecimento não identificado",date:draft.data||dateOnly(new Date()),total:Number(draft.total)||0,confidence:draft.confianca,notes:draft.observacoes,items:(draft.itens||[]).map(i=>({id:uid(),description:i.descricao||"Item não identificado",quantity:Number(i.quantidade)||1,unit:i.unidade,unitPrice:i.valorUnitario==null?undefined:Number(i.valorUnitario),total:Number(i.valorTotal)||0,macroCategory:i.categoriaMacro||groceryMacro(i.descricao||"")}))};mutate(d=>{(d.receipts??=[]).push(receipt)});setDraft(undefined);setMessage("Nota adicionada ao histórico. Clique em Salvar para sincronizar no OneDrive.")};
  const history=data.receipts||[];const allPurchases=history.flatMap(r=>r.items.map(i=>({r,i})));const keys=Array.from(new Set(allPurchases.map(x=>normalize(x.i.description))));const products=keys.map(key=>{const purchases=allPurchases.filter(x=>normalize(x.i.description)===key).sort((a,b)=>a.r.date.localeCompare(b.r.date));const last=purchases.at(-1);const intervals=purchases.slice(1).map((x,index)=>(new Date(x.r.date).getTime()-new Date(purchases[index].r.date).getTime())/864e5);const averageDays=intervals.length?Math.round(intervals.reduce((a,b)=>a+b,0)/intervals.length):undefined;const next=averageDays&&last?new Date(new Date(last.r.date).getTime()+averageDays*864e5).toISOString().slice(0,10):undefined;return{name:last?.i.description||key,category:last?.i.macroCategory||groceryMacro(last?.i.description||key),count:purchases.length,averageQuantity:purchases.reduce((s,x)=>s+x.i.quantity,0)/purchases.length,price:last?.i.unitPrice??last?.i.total,store:last?.r.store,averageDays,next}}).sort((a,b)=>b.count-a.count);
  return <>
    ...
    <section className="grid two"><div className="panel"><h2>Histórico de compras</h2>{history.length?history.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(r=><div className="budget-item" key={r.id}><div><b>{r.store}</b><small>{r.date} · {r.items.length} itens · {money(r.total)}</small></div><button onClick={()=>mutate(d=>{d.receipts=(d.receipts||[]).filter(x=>x.id!==r.id)})}><Trash2 size={15}/></button></div>):<Empty/>}</div><div className="panel"><h2>Sugestões de reposição</h2><p className="muted">Calculadas pelo intervalo e quantidade média das compras.</p>{products.filter(p=>p.next).map(p=><Row key={p.name} a={p.name} b={`${p.category} · média ${p.averageQuantity.toFixed(1)} un. · a cada ${p.averageDays} dias`} c={p.next||"—"}/>)}</div></section>
    <section className="panel"><h2>Todos os produtos comprados</h2><p className="muted">Catálogo consolidado com categoria macro, frequência, quantidade e último preço.</p>{products.length?products.map(p=><Row key={p.name} a={p.name} b={`${p.category} · ${p.count} compra(s) · média ${p.averageQuantity.toFixed(1)} un. · ${p.store||""}`} c={p.price==null?"—":money(p.price)}/>):<Empty/>}</section>
  </>;
}
