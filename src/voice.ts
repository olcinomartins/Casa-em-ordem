...
export async function readVoiceExpense(blob:Blob):Promise<VoiceTransaction>{
  if(blob.size>18_000_000)throw new Error("O áudio deve ter menos de 18 MB.");
  const response=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${await getMicrosoftAccessToken()}`},body:JSON.stringify({audio:await toBase64(blob),mimeType:blob.type||"audio/mp4"})});
