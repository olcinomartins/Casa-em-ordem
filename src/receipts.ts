...
export const authorizeReceiptReader=()=>window.open(endpoint,"gemini-access","width=520,height=720");
export async function readReceipt(file:File):Promise<ReadReceipt>{
  ...
  const response=await fetch(endpoint,{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({mimeType:file.type,image:await toBase64(file)})});
